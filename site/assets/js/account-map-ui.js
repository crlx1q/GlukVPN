(function(D){
 ...
 // Ready-made Material glyphs, identical to Flutter Icons.*, exported to PNG.
 ...
 D.drawAccountMap=function(data){
  ...
  rows.forEach(function(d){var n=d.node,location=n&&n.location,server=n&&([n.city,n.country].filter(Boolean).join(', ')||n.name)||(en?'Unknown node':'Узел неизвестен'),label=(d.deviceName||'Device')+' · '+(d.platform||'—')+' → '+server;
   if(d.origin&&d.origin.source==='device-estimate')label+=(en?' · ≈ device region estimate':' · ≈ оценка региона устройства');
