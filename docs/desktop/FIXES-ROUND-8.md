...
| Пункт | Состояние |
| --- | --- |
| Баннер обновления в интерфейсе | сервис готов и протестируем, но сам баннер в `desktop_shell` / Android / попапе расширения ещё не нарисован |
| Скрытое меню разработчика (5 кликов по версии) | не сделано. На Desktop канал сейчас задаётся при сборке (`--dart-define=GLUK_CHANNEL`), для рантайм-переключения нужно провести канал через `DesktopSettings` и `ApiClient` |
