...
форма первой. `id` — код самой платёжки (у Cashera `sbp`/`card`/`crypto`),
он же уходит обратно в заказ. Список пуст, когда шлюз эту валюту не берёт,
и состоит из одного `all`, когда выбор способа делает сама страница платёжки
(TabPay, MulenPay) — сайт тогда селектор не рисует. `minimumMinor` —
...
```json
{
  "billingEnabled": true,
  "provider": "tabpay",
  "methods": [{ "id": "all", "label": "Все способы" }],
  "show": false,
  "trial": {
    "enabled": true,
    "planCode": "basic",
