...
## Шестой проход: тренды, кольцо лимита и тултипы (2026-09-13, задача #025)

Макет требовал одного на всех площадках: «+12 %» у итогов, кольцо вместо
полоски у лимита тарифа, тултип с цифрами на графике и пик с датой.
Всё это должно было появиться, не превращая клиентский рендер в источник цифр.

### Сервер: окно сравнения считает API

- `services/insightMath.ts`: `previousWindow(period, now)` даёт прошлое окно той же
  длины с концом `min(start + elapsed, currentStart)`, `trendPercent(current, previous)` —
  целый процент либо `null` при `previous <= 0`.
- `services/accountInsights.ts` собирает бакеты обоих окон **одним** `findMany`
  (граница сдвинута влево до `previous.start`); строки до текущего окна
  по-прежнему попадают только в `earlier`.
- Новые поля ответа — `previous` и `trend`; `trend.comparable` равен `true` только
  когда `coverage.since` не позже `previous.start`. Контракт — в `docs/api.md`.
- Проценты считает **только** сервер: клиент, считающий тренд сам,
  разошёлся бы с бакетами узла — то же правило, что и для расхода
  (`docs/traffic-integrity.md`).

### Клиенты

| Площадка | Где | Что появилось |
| --- | --- | --- |
| Сайт | `site/assets/js/sprint2.js`, `assets/css/sprint2.css` | чип окна, спарклайны, бейджи тренда, тултип с точками, подпись пика |
| ПК и телефон | `lib/widgets/usage_stats.dart`, `widgets/quota_bar.dart`, `models/account_insights.dart` | кольцо лимита, бейджи тренда, чип шага, пик с датой |
| Расширение | `extension/ui/popup.js`, `ui/theme.css` | бейджи тренда, тултип с точками, пик с датой |

- **Сайт.** `renderAnalytics` = чип фактического окна + шкала лимита + янтарная
  плашка + четыре карточки (Получено / Отправлено / Всего — со спарклайном
  и бейджем, Покрытие — текстом) + график. Весь JS сайта остаётся ES5:
  `site/tests/site.test.cjs` прогоняет каждый файл через `new vm.Script`.
  Версии ассетов подняты до `?v=20260913.1` в `site/app/index.html` и
  `site/en/app/index.html`; `dashboard.css?v=20260909.1` намеренно **не** бампили —
  эту строку пинит тест.
- **Flutter.** `QuotaBar` получил флаг `ring` (по умолчанию `false`, компактная
  шкала на главном экране не изменилась). На экране статистики кольцо 62×62
  рисует `_RingPainter` (`SweepGradient` от 12 часов), процент в центре, дата
  сброса словами («5 октября») без `intl`. Модель терпима к старым ответам:
  `previous` необязателен, `trend` по умолчанию `TrendInfo.none`, существующий
  `test/account_insights_test.dart` проходит без правок. ПК и телефон рендерят
  один `UsageStatsView`, поэтому обе площадки закрыты одной правкой.
- **Расширение.** Бейджи в карточках итогов, пик подписан временем бакета,
  svg-подсказки `<title>` заменены на свой тултип. Всё собирается через
  `createElement`: MV3 CSP закрывает `innerHTML`.
- Общее решение для сайта и расширения: **тултип, вертикаль и точки — HTML
  поверх svg**, а не внутри него. График растянут `preserveAspectRatio="none"`,
  и любой текст или кружок внутри svg растянулся бы вместе с сеткой.
  Полосы-хитзоны получают `tabindex` и `aria-label`, показ — на чистом CSS
  по `:hover` / `:focus-visible`, без скрипта и без мыши.

### Сознательные отличия от макета

- **Произвольный выбор диапазона дат** не сделан: API знает только
  `period=day|week|month`, а календарь, который сервер не исполняет, — обман
  интерфейса. Вместо него чип с фактическим окном (`start → end · шаг · UTC`).
  Задача осталась в бэклоге `docs/NEXT_AGENT_PROMPT.md`.
- **Дропдаун «По дням»** из макета стал неинтерактивным чипом: шаг бакета
  выбирает сервер по периоду, выпадающий список притворялся бы настройкой.
- Подпись тренда — «к прошлым суткам / к прошлой неделе / к прошлому месяцу»
  вместо макетного «за выбранный период»: сравнение идёт с прошлым окном,
  а не внутри текущего.
- Бейджа нет вообще, пока `trend.comparable !== true`. Новый аккаунт видит
  янтарную плашку про неполную историю и ни одного процента.

### Verification (шестой проход)

Как и в прошлые проходы — только статика: в песочнице нет ни `npm`,
ни Flutter SDK, ни один набор тестов не запускался. Проверено чтением
и поиском: класса `stats-hit` больше нет ни в одном файле; тесты расширения
(`extension/tests/sprint2-contracts.test.mjs`) статистику не пинят; тесты Flutter
не пинят строк квоты и пика; `dashboard.css?v=20260909.1` не тронут.

Перед релизом нужны: `npm run typecheck` и `npm test` в корне,
`npm test --prefix site`, `node --test glukvpn-extension-1.5.0/extension/tests/sprint2-contracts.test.mjs`,
`flutter analyze` + `flutter test`, затем визуальная проверка на 320/390/900 px,
в попапе Chrome, на Windows и Android — обязательно на двух аккаунтах:
с историей короче выбранного периода (бейджей быть не должно) и с полной.
Миграция базы для этого прохода не требуется: оба окна читаются из тех же
бакетов `TrafficUsageBucket` (ретеншн 400 дней).

## Седьмой проход: полные окна графиков и роль support (2026-09-13, задача #026)

Два независимых требования. Первое: график должен показывать окно целиком —
день все прошедшие часы, неделя 7 дней, месяц 30, и всё это должно
умещаться в ширину карточки. Второе: в админке нужна роль саппорта —
смотреть можно всё, менять — только подписку, и кнопка выдачи самих ролей.

### Сервер: сетка серии и флаг `isSupport`

- `services/accountInsights.ts`: сетка серии строится от `usageBucket(window.start, …)`
  до `now`, а не от первого бакета с данными. `coverage.since` больше не режет
  левый край — он нужен только для `coverage.partial` и `trend.comparable`.
  Раньше аккаунт, у которого трафик пошёл два часа назад, видел график из двух
  бакетов на всю ширину и выглядело это как полные сутки.
- Окна остались календарными UTC (сутки с 00:00, неделя с понедельника, месяц
  с первого числа); `usageWindow` и `previousWindow` не трогали. Скользящие окна
  («последние 24 часа») отклонены сознательно: у них нет фиксированной
  границы, и сумма на экране никогда не сошлась бы с расчётом квоты.
- Новый флаг `isSupport` (`is_support`, `@default(false)`) в `users` рядом с `isTester`;
  миграция `20260913120000_user_support_flag` (`ADD COLUMN IF NOT EXISTS`).
  Флаг попадает в `userPayload` (т.е. в ответ логина), в `AccessTokenPayload.sup`
  и сбрасывается в `accountDeletion.ts` вместе с остальными привилегиями.
- `requireStaff` вместо `requireAdmin` на всём `/api/admin/*` плюс второй `preHandler`
  с двумя списками: `SUPPORT_DENIED_READS` (только `traffic-budget`) и
  `SUPPORT_ALLOWED_WRITES` (только `users/:id/subscription`). Запись именно
  allow-list'ом: новая ручка по умолчанию admin-only, и забытый deny-list не может
  тихо расширить права саппорта.
- Новые `POST /api/admin/users/:id/admin` и `…/support` (тело `{ enabled }`, аудит
  `admin.user.admin` и `admin.user.support`). Свою админку снять нельзя (409),
  `DELETED`-аккаунту роли не выдаются (409). Контракт и матрица прав —
  в `docs/api.md`, раздел «Роли admin и support».

### Клиенты

| Площадка | Где | Что появилось |
| --- | --- | --- |
| Сайт | `site/assets/js/sprint2.js`, `assets/css/sprint2.css` | подписи под графиком (`tickLabel` / `axisTicks`, до 6 меток) и figcaption с окном |
| ПК и телефон | `lib/widgets/usage_stats.dart` | `_tickIndices()` в `_TrafficChart`: ось рисует выборку, а не все бакеты |
| Расширение | `extension/ui/popup.js` | `stats-axis` до 5 подписей (`tickIndexes` / `statsTick`), «UTC» у последней |
| Админка | `control-server/public/admin.js`, `public/index.html` | ролевой режим, колонка «Support», кнопки выдачи ролей |

- **Подписи оси везде разреживаются, а не сжимаются.** 30 дневных меток в
  попапе шириной 360 px неразборчивы, поэтому подписей фиксированное
  малое число, а бакеты рисуются все. Сами столбики не прореживаются: дыра
  в серии читалась бы как ноль трафика.
- Сайт остаётся ES5 (`site/tests/site.test.cjs` гоняет каждый файл через
  `new vm.Script`), поэтому в `sprint2.js` ни стрелок, ни `const`. Версии ассетов
  подняты до `?v=20260913.2` в `site/app/index.html` и `site/en/app/index.html`;
  `dashboard.css?v=20260909.1` снова **не** трогали — эту строку пинит тест.
- **Админка.** Вся разница ролей сводится к одному хелперу `canManage()`
  (`state.isAdmin === true`) и к `applyRoleVisibility()`, который прячет вкладки
  «Channels» и «Billing», сервисные блоки, создание пользователей, enrollment ноды
  и кнопки чистки. Саппорт видит те же таблицы, но без кнопок: у нод в
  колонке действий тире, у устройств и сессий нет Revoke и Close, в строке
  пользователя остаётся только переключатель подписки.
- Саппорту `loadAll()` не дёргает админские разделы второго эшелона (грузится
  только `loadClientErrors()`): любой 403 в общем `Promise.all` попадал в catch и
  выкидывал его из панели через `signOut()`.
- Логин панели пускает `isAdmin || isSupport`, запоминает оба флага и показывает
  роль в строке `who`; в `signOut()` флаги сбрасываются, иначе следующий вход
  отрисовался бы с правами предыдущего аккаунта.

### Сознательные отличия и границы роли

- Саппорт — не «админ поменьше»: ноды, обслуживание, каналы, сервисные
  настройки, блокировки, удаление, скоростной лимит и любые флаги (включая
  свой собственный) остаются за админом.
- Бюджет egress был закрыт для саппорта даже на чтение — отменено восьмым
  проходом: расход трафика нужен поддержке в работе, а границу роли
  держит allow-list записи, а не спрятанные цифры.
- Прятать кнопки в UI — удобство, а не защита. Права решаются на сервере;
  `curl` в обход панели получит 403.
- В веб-кабинете пользователя роль support никак не отображается: это
  внутренняя роль персонала, а не свойство аккаунта для самого клиента.

### Verification (седьмой проход)

Как и раньше — только статика: в песочнице нет ни `npm`, ни Flutter SDK,
ни один набор тестов не запускался и миграция не применялась. Проверено
чтением и поиском: `requireAdmin` в `routes/admin.ts` больше не используется,
в `admin.js` каждая мутирующая кнопка под `canManage()`, colSpan пустой таблицы
поднят до 10 под новую колонку, ассеты из HTML сайта существуют, порядок
скриптов и `dashboard.css?v=20260909.1` не тронуты.

Перед релизом нужны: `npx prisma migrate deploy` (или `migrate dev`) для
`20260913120000_user_support_flag` — без неё сервер упадёт на первом же
запросе к `users`; далее `npm run typecheck` и `npm test` в корне,
`npm test --prefix site`, `node --test glukvpn-extension-1.5.0/extension/tests/sprint2-contracts.test.mjs`,
`flutter analyze` + `flutter test`. Ручная проверка: графики день/неделя/месяц
на 320/390/900 px и в попапе (подписи не налезают, окно целое), и вход в
админку тремя аккаунтами: админ (всё на месте), саппорт (только подписка,
нет вкладок каналов и биллинга; `traffic-budget` после восьмого прохода
отвечает 200), обычный юзер (403 и отказ на логине в панель).

## Восьмой проход: фильтр пользователей, честные устройства, одна шкала квот (2026-09-13, задача #027)

### Фильтр списка пользователей

- `GET /api/admin/users?filter=` принимает `active`, `disabled`, `blocked`,
  `deleted`, `admins`, `support`, `testers`, `all`. По умолчанию и на любой мусор
  в query — `active`: удалённые аккаунты остаются в базе надгробиями ради
  статистики трафика и в общем списке только шумят.
- Срезы по ролям (`admins`, `support`, `testers`) исключают `DELETED`, иначе
  снятый аккаунт всплывал бы в роли. Ответ — `{ users, filter }`, где `filter` —
  фактически применённое значение, чтобы селект не расходился с данными.
- В панели — селект `#user-filter` в шапке таблицы, `state.userFilter` по
  умолчанию `"active"`, пустой результат пишет «No users in this filter.»:
  «пустая база» и «суженный фильтр» — разные сообщения.
- Роль бета-тестера оставлена как есть: она управляет только переключателем
  PROD/BETA на клиентах и ничего больше не даёт.

### Одна палитра rx/tx

- Эталон — расширение: `#3ddc97` (зелёный) = получено клиентом (download),
  `#c4b5fd` (фиолетовый) = отправлено клиентом (upload).
- На сайте цвета были перевёрнуты: в `sprint2.css` свопнуты `.is-dn` / `.is-up`
  и тона тултипов (`#9ceec4` / `#c9bffb`). Во Flutter палитра уже совпадала.
- В админке подписи разведены словами: `rx` — clients upload, `tx` — clients
  download, потому что на сервере эти буквы означают обратное клиентским.
- Не тронуты похожие по цвету, но другие по смыслу элементы:
  `.s2-trend.is-up/.is-down` (рост/падение), `.s2-peak`, линии маршрутов на
  карте и `.dash-bar` (остаток оплаченного периода, а не квота).

### Одна шкала квот: зелёный → жёлтый → красный

- Градиенты «зелёный → синий/фиолетовый» убраны везде: цвет теперь несёт
  смысл, а не украшение. Hue: 142 до 50 %, дальше линейно 142→50 к 70 %,
  50→6 к 90 %, 6→0 на 100 %. Заливка — `linear-gradient(90deg,
  hsl(H 68% 46%), hsl(H 70% 52%))`, сплошной цвет — `hsl(H 70% 52%)`.
- Имена функций одинаковы на всех площадках — `quotaHue`, `quotaFill`,
  `quotaColor` (`public/admin.js`, `site/assets/js/sprint2.js`,
  `extension/ui/popup.js`); во Flutter это `QuotaScale.tone()` и
  `QuotaScale.toneFromPercent()` в `lib/theme/tokens.dart`.
- Пороговые классы удалены (`.warm`, `.is-warn/.is-high/.is-crit`,
  `.is-over` на заливке): ступеньки врали на границах. В CSS остались
  одноцветные fallback'и на случай выключенного JS.
- Той же шкалой красятся бары бюджета egress и загрузки нод: «мало
  осталось» выглядит одинаково в кабинете, расширении, клиенте и панели.
- В `site/assets/js` использован запятая-синтаксис `hsl(H,68%,46%)` и только
  ES5: тест сайта парсит эти файлы через `new vm.Script`.
- В расширении и админке цвет ставится через CSSOM (`el.style.*`), без
  `innerHTML` и inline-атрибутов в разметке — требование CSP MV3.

### Саппорт видит Oracle egress

`SUPPORT_DENIED_READS` пуст, `applyRoleVisibility()` больше не скрывает
`#egress-block`, и саппорту он грузится вместе с журналом клиентских ошибок.
Записи не изменились: выключить или удалить ноду он по-прежнему не может.

### «Devices active 3 / 54»

- 54 — это ACTIVE плюс REVOKED-надгробия: каждый выход и повторный
  enrollment оставляют строку, чтобы прошлый трафик остался привязан.
- `GET /api/admin/overview` отдаёт `devices: { active, revoked, total }`; панель
  показывает `active / active`, а надгробия — отдельной строкой
  «+N revoked records kept for traffic stats». В строках юзеров `devices` и
  `sessions` тоже считают только активные.
- Чистка: `purgeStaleDevices` в мониторе (окно ≥ 30 дней) и кнопка →
  `DELETE /api/admin/devices/stale?days=N`; активные устройства не трогает.

### Почему rx 3.18 GB / tx 13.18 GB ≠ Oracle 44.61 GB

- rx/tx в панели — со стороны узла: rx = принято от клиента (его upload),
  tx = отдано клиенту (его download). Сравнивать с Oracle надо сумму (≈ 16.4 GB),
  а не одну колонку.
- Oracle считает `VnicBytesOut` по всему компартменту (фильтр по VNIC
  включается только через `OCI_EGRESS_VNIC`): туда попадают сайт, API, панель,
  бот, обновления ОС, мониторинг, бэкапы, а также инкапсуляция WG/VLESS,
  ретрансмиты и keepalive.
- Релей удваивает egress: download клиента — egress к клиенту, upload клиента
  — egress в интернет.
- Окна разные: Oracle — текущий биллинг-цикл от `OCI_BILLING_CYCLE_START`,
  панель — сумма по сессиям, ещё живущим в БД.
- Два источника недоучёта в панели: `Session.device` с `onDelete: Cascade` —
  `purgeStaleDevices` уносит байты вместе с надгробиями; WG-счётчики берутся
  как `max(старое, новое)`, поэтому после рестарта wg или узла трафик до
  прежнего максимума теряется.
- Вывод: расхождение ожидаемое. Для сверки до второго знака нужно задать
  `OCI_EGRESS_VNIC` и сравнивать на окне биллинг-цикла.

### Verification (восьмой проход)

Снова только статика: ни npm, ни Flutter SDK в песочнице нет, тесты и
typecheck не запускались, миграция `20260913120000_user_support_flag`
по-прежнему не применена. Проверено чтением и grep-ом: `site.test.cjs` не
пинит ни цветовые хексы, ни `load-bar` / `is-quota` / `is-budget` / `warm`;
`dashboard.css?v=20260909.1`, порядок скриптов и список ассетов не тронуты;
colSpan заглушек таблицы пользователей — 10 под текущие колонки.

Перед релизом вручную: все значения фильтра (включая битый `?filter=чушь`
— должен дать `active`), шкалы на 40 / 60 / 80 / 95 / 105 % на сайте, во
Flutter, в попапе и в панели (цвета совпадают, скачков нет), и вход
саппортом: блок Oracle виден, кнопок нод и чисток нет.

## Девятый проход: формулировки сайта, реальный итог оплаты, регистрация без Telegram (2026-09-14, внеплановая задача)

Банк отказал магазину TabPay по двум причинам: на сайте рекламировался обход
блокировок, а после оплаты подписка фактически не выдавалась. Плюс владелец
попросил убрать «отчётные» фразы-заглушки и снять обязательную привязку
Telegram при регистрации — в РФ мессенджер недоступен.

### Копирайт: сервисы вместо блокировок

- `site/index.html`, `site/en/index.html`, `site/features/index.html`,
  `site/en/features/index.html`: eyebrow секции доступа → «Скорость в
  сервисах», заголовок → «Видео, музыка и игры — без лагов», lead
  рассказывает про скриншоты Android-приложения. Слова про обход блокировок
  и цензуру из продающих текстов убраны: продукт продаётся как приватность и
  скорость, а не как способ что-то обойти.
- `id="access-title"` и структура секции сохранены, ссылок и якорей
  `#access` в проекте нет — правки чисто текстовые.
- Самореферентные заглушки («Это не стоковые картинки…», «без маркетингового
  шума» / «free of marketing noise») удалены: текст, который объясняет сам
  себя, читается как отчёт ИИ, а не как страница продукта.
- Осознанно не тронуто: юридические тексты `site/terms`, `site/en/terms` и
  секция «Чего VPN не делает» — это обязательства перед пользователем и
  платёжной системой, а не реклама обхода.

### Оплата: ожидание → подключено / отклонено

- `site/assets/js/app.js`: после возврата с TabPay фронт больше не показывает
  вечное «ожидание». `gatewayReturn()` вызывает `POST /api/billing/orders/sync`
  до пяти раз с шагом 3 с: редирект и вебхук гоняются, и без ретраев
  пользователь видел старый статус заказа.
- Исход разведён на три сообщения: подписка подключена, платёж отклонён
  (`FAILED` / `CANCELED` / `CANCELLED` / `EXPIRED`) и «ещё обрабатывается».
  Query-параметры затираются, чтобы перезагрузка не поднимала баннер заново.
- Блок написан в ES5 и вставлен перед `loadAll()`: `site/tests/site.test.cjs`
  парсит `site/assets/js/*.js` через `new vm.Script`. Пять строк добавлены в
  `site/assets/js/i18n.js` с EN-переводами.
- `control-server/src/services/billing.ts`, `reconcilePendingOrders`: к
  PENDING-заказам за 7 дней добавлены недавние FAILED / CANCELLED за 24 ч и
  `markOrderPaid({ revive: true })`. TabPay иногда сначала помечает заказ
  неуспешным, а затем присылает подтверждение — без ревайва оплаченный
  заказ оставался мёртвым.

### Регистрация: почта и код, Telegram потом

- Флаг `REGISTER_REQUIRE_TELEGRAM` (`control-server/src/config.ts`, по
  умолчанию `false`) решает, обязателен ли Telegram. Прежняя воронка не
  удалена: включение флага возвращает старое поведение без правки кода.
- `services/registration.ts`: при выключенном флаге `confirmRegistrationEmail`
  сразу создаёт пользователя (`createUserFromPending`: уникальный username,
  `withRegistrationGate`, IdentityLink GOOGLE при `pending.googleSub`,
  `grantDefaultSubscription`) и отвечает `state: "done"` с `verified: false`.
- Повторный submit кода не падает: сначала ищем существующего User по
  email, а гонку двух одновременных запросов ловим в catch и перечитываем
  запись. `registrationStatus` тоже самовосстанавливается в этом режиме.
- Уведомление владельцу вынесено в `notifyNewAccount()` и переиспользовано из
  `attachTelegram()`, чтобы обе воронки слали один и тот же текст.
- `routes/register.ts`: `/api/auth/config` отдаёт `telegram.required` отдельно
  от `telegram.enabled` — фронту нужно различать «бот выключен» и «бот не
  обязателен»; 503 при недоступном боте срабатывает только когда Telegram
  действительно обязателен.
- `services/accountView.ts` → `userPayload`: добавлены `telegramLinked`,
  `telegramVerified`, `verified`. Одно поле `verified` решает, что рисовать
  клиентам, вместо вывода из трёх других.
- Фронт: `register.js` через `showDone(username, unverified)` показывает
  подсказку `#reg-done-tg` (добавлена в `site/login/index.html` и
  `site/en/login/index.html`), `account.js` различает три состояния:
  привязан и подтверждён, привязан без подтверждения, не привязан.
- Триал остался за Telegram: `TRIAL_OFFER_REQUIRE_TELEGRAM` по умолчанию `true`,
  иначе бесплатный период легко фармить одноразовыми почтами.

### Verification (девятый проход)

Снова только статика: npm и Flutter SDK в песочнице нет, тесты, сборка и
typecheck не запускались, миграция `20260913120000_user_support_flag`
всё ещё не применена. Проверено чтением и grep-ом: `site.test.cjs` не пинит
изменённые строки, `dashboard.css?v=20260909.1`, порядок скриптов и список
ассетов не тронуты, новый код в `site/assets/js` держится ES5, класс
`auth-hint` существует в `site/assets/css/auth.css`.

Перед релизом обязательно: задать `TABPAY_WEBHOOK_SECRET` — при пустом
секрете `verifyTabpaySignature` всегда даёт false и вебхук не выдаст
подписку, какими бы точными ни были фронтовые ретраи; прописать в
дашборде TabPay URL `…/api/billing/webhook/tabpay`; перевести
`BILLING_SUCCESS_URL` и URL отказа на `/app/` — сейчас отказ ведёт на
`/pricing/?failed=1&order=`, где обработчика нет; прогнать регистрацию на
живом стенде в обоих режимах флага, включая двойной submit кода и
Google-воронку (`GOOGLE_REQUIRE_TELEGRAM` — отдельный флаг, не менялся).
