...
- Бюджет egress закрыт для саппорта даже на чтение: это данные о затратах,
  а не материал поддержки.
...
нет вкладок каналов и биллинга, `traffic-budget` даёт 403), обычный юзер (403
и отказ на логине в панель).
