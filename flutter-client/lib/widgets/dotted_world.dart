		...
		if (!(accountArcs ?? const <ConnectionArc>[]).any((a) => a is AccountConnectionArc && a.isCurrent) && self != null && server != null && arcProgress > 0) {
			...
			if (routeFade > 0.02) {
				_paintArc(
					...
					opacity: routeFade,
					accent: accent,
					// Это локальная нить «я → выбранный сервер»: её рисуем до того,
					// как сервер вообще узнает о сессии. Без progress она
					// появлялась сразу целиком, и фаза «подключаемся» на телефоне
					// вообще не читалась — теперь она вырисовывается и втягивается
					// так же, как на ПК.
					progress: arcProgress.clamp(0.0, 1.0),
				);
			}
