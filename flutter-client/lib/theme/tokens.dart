...
/// Единая шкала «сколько осталось» для квот, бюджетов и загрузки узлов.
///
/// Прежние шкалы меняли цвет ступенькой на 90 %, поэтому полоса выглядела
/// одинаково и на 5 %, и на 85 % — предупреждение приходило, когда менять
/// поведение уже поздно. Теперь оттенок едет непрерывно: зелёный до 50 %,
/// жёлтый к 70 %, красный к 90 %.
///
/// Тот же расчёт продублирован в админке (`public/admin.js`), на сайте
/// (`assets/js/sprint2.js`) и в расширении (`ui/popup.js`) — пороги нужно
/// менять сразу во всех четырёх местах, иначе одна и та же квота будет
/// разного цвета в телефоне и в браузере.
class QuotaScale {
	QuotaScale._();
		...
		final double hue = p <= 50
				...
				: p <= 70
						...
						: p <= 90
								? 50 - ((p - 70) / 20) * 44
								: 6 - ((p - 90) / 10) * 6;
		return HSLColor.fromAHSL(1, hue, 0.70, 0.52).toColor();
	}

	/// То же самое для данных, которые сервер отдаёт сразу в процентах.
	static Color toneFromPercent(num percent) => tone(percent.toDouble() / 100);
}
