...
class _MapBackdropState extends State<_MapBackdrop>
  ...
  Widget build(BuildContext context) {
    ...
    // ЭТАП 4: кадр снова адаптивный, но адаптируется к паре «я → сервер».
    //
    // История важна, иначе фикс поедет назад. `fitConnections`
    // пересчитывал И зум, И центр по рамке всех точек, а список устройств
    // приходит опросом: любое изменение давало новую рамку, и камера
    // ехала к ней — это и выглядело как «фигуры резко телепаются
    // вправо-влево». Тогда зум зафиксировали, но вместе с ним пропало и
    // то, о чём просят теперь: далёкий сервер уезжал под показания или
    // за кадр, а вертикаль камеры вообще не зависела от точек —
    // `topAnchored` берёт только `centreOn.fx`.
    //
    // Теперь рамку считает `FlatMapView.framePoints`: масштаб по обоим
    // измерениям сразу, сдвиг только вверх и не ближе roomyCoverage, а
    // чужие устройства влияют на кадр лишь пока не мешают паре.
    //
    // Пара, ради которой кадр и существует: это устройство и выбранный
    // сервер. Сервер известен ещё до подключения (`vpn.selectedNode`),
    // поэтому маршрут виден до нажатия CONNECT, а не после.
    final List<MapPoint> pair = <MapPoint>[
      selfPoint,
      if (serverPoint != null) serverPoint,
    ];
    final List<MapPoint> withFleet = <MapPoint>[...pair];
    // Концы живых нитей аккаунта хорошо бы тоже видеть: иначе от
    // далёкого устройства в кадре висит половина нити, уходящая в
    // никуда. Но последнее слово остаётся за парой — см. выбор кадра
    // ниже.
    for (final ConnectionArc arc in widget.accountArcs) {
      withFleet.add(arc.from);
      withFleet.add(arc.to);
    }
    final Size viewport = MediaQuery.sizeOf(context);
    // Два кадра: обязательный (пара) и желаемый (пара + чужие устройства).
    final FlatMapView pairView = FlatMapView.framePoints(
      viewport: viewport,
      points: pair,
      topPadding: -6,
    );
    final FlatMapView fleetView = FlatMapView.framePoints(
      viewport: viewport,
      points: withFleet,
      topPadding: -6,
    );
    // Далёкое чужое устройство не имеет права уводить камеру так далеко,
    // что пара «я → сервер» превращается в две точки рядом: если ради
    // него пришлось бы отъехать больше чем на четверть, показываем только
    // пару. Это же держит кадр спокойным, когда устройства приходят и
    // уходят опросом.
    final FlatMapView view =
        fleetView.zoom >= pairView.zoom * 0.75 ? fleetView : pairView;
