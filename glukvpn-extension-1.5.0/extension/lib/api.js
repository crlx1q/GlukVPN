...
export const Api = {
	...
	/*
	 * Adopts a refresh token that content/site-bridge.js lifted from the website.
	 *
	 * background.js has always called this, but the method never existed, so
	 * "Sign in with the website" threw every single time. The token belongs to one
	 * specific channel, so the exchange is pinned to that channel's base URL
	 * rather than whatever happens to be configured right now.
	 */
	async adoptRefreshToken(channel, refreshToken) {
		const token = String(refreshToken ?? '').trim()
		if (!token) {
			throw new ApiError({ statusCode: 0, code: 'no_token', message: 'No sign-in token found on the website.' })
		}
		const settings = await Store.settings()
		const base = String(settings.apiBase?.[channel] ?? settings.apiBase?.prod ?? '').replace(/\/+$/, '')
		if (!base) {
			throw new ApiError({ statusCode: 0, code: 'no_base', message: 'No API endpoint configured for this channel.' })
		}

		const json = await request('POST', '/api/auth/refresh', {
			absoluteUrl: `${base}/api/auth/refresh`,
		...
		// /api/auth/refresh does not always echo the profile, and the popup needs a
		// name to show. A failure here must not undo a successful sign-in.
		if (!json.user) {
			try {
				const profile = await request('GET', '/api/auth/me', { absoluteUrl: `${base}/api/auth/me` })
				const session = (await Store.session()) ?? {}
				await Store.saveSession({
					...session,
					user: profile.user ?? profile ?? null,
					subscription: profile.subscription ?? session.subscription ?? null,
				})
			} catch {}
		}
